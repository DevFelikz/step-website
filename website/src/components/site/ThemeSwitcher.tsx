...
function getStoredTheme(): "dark" | "light" {
  ...
  return (localStorage.getItem("step-theme") as "dark" | "light") ?? "dark";
}
...
function applyTheme(theme: "dark" | "light") {
  ...
  localStorage.setItem("step-theme", theme);
}
...
export function ThemeSwitcher() {
  ...
  if (!mounted) return <div className="h-8 w-[3.75rem] rounded-full" aria-hidden />;
    ...
    >
      ...
      <span
        className={`relative z-10 flex h-6 w-6 items-center justify-center rounded-full shadow-sm transition-all duration-300 ${
          isDark ? "translate-x-0 bg-step-card" : "translate-x-[1.75rem] bg-step-gold"
        }`}
        aria-hidden
      >
...
function SunIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 text-step-gold" aria-hidden>
...
function MoonIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="currentColor" className="h-3.5 w-3.5 text-step-muted" aria-hidden>
